...
const AutocompleteItem = ({ _id, empresa, usuario, ocEmpresa, proveedor, productos, fechaCompra, total, cantidadItems, alias, ingresadoAlmacen, updatedAt, setAutoCompleteState, formValues, setFormValues,  criterio }) => {
    ...
    const desglozaCantidad = (producto) => {
      ...
      if (producto.usaSerie){
        for(let i = 0; i < producto.cantidad; i++){
          ...
          producto.detalle[i].serieSinRepetir = false
    ...
    const productosActuales = (productosPoblar) => {
        const productosActuales =  productosPoblar?.map((producto) => {
          if(producto.usaSerie){
            ...
            for(let i = 0; i < producto.cantidad; i++){
              producto.detalle[i].errorSerie = false
              producto.detalle[i].serieValidada = true
              producto.detalle[i].serieSinRepetir = true
              producto.detalle[i].mensajeError = ""
          ...
          //   ...producto,
          //   detalle: desglozaCantidad(producto)
          // }
        })
      ...
      const response = await fetch(`/api/compras/ordenCompra/buscar/${_id}`)
        ...
        <div className='w-4/5 flex justify-between ' >
          <div>
            ...
            <h3 className='text-sm font-semibold'>
              OC: {String(ocEmpresa).padStart(6, '0')}
              {isOCNoPermitido && (
            ...
            <p className="mt-1 truncate text-xs text-gray-500"><strong>Proveedor:</strong>  {proveedor.razonSocial}</p>
            <p className="mt-1 truncate text-xs text-gray-500"><strong>{proveedor.docId.nombre}:</strong>  {proveedor.numeroDoc}</p>
          </div>
...
export default function SearchOCMovimientoAlmacen(props) {
    ...
    const autoComplete = useMemo(() =>   createAutocomplete({
        placeholder: "Ingresa busqueda de Orden Compra",
        onStateChange: ({ state }) =>  {
          setCriterio(state)
            ...
            getItems: async({ query }) => {
              if (!!query && query.length >= minLetras) {
                return toast.promise(
                  ...
                  fetch(`/api/inventario/movimientoAlmacen/buscar/ocEmpresa?q=${query}&idempresa=${props.empresaSeleccionada.empresaRef}`)
                    .then((res) => res.json()),
                  {
