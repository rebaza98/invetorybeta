...
const AutocompleteItem = ({ _id, empresa, usuario, regulacionEntradaEmpresa,  productos, fechaRegulacion, total, cantidadItems, updatedAt, alias, ingresadoAlmacen, setAutoCompleteState, formValues, setFormValues,  criterio }) => {
  ...
  const handleItemClick = async () => {
    ...
    const productosActuales = (productosPoblar) => {
      ...
      const response = await fetch(`/api/regulaciones/regulacionEntrada/buscar/${_id}`)
        ...
        setFormValues({
          ...
          proceso: [
            ...formValues.proceso,
            {
              idProceso: _id,
              empresa: empresa,
              regulacionEntradaEmpresa: regulacionEntradaEmpresa,
              usuario: usuario,
        ...
        <div className='w-4/5 flex justify-between ' >
          <div>
            ...
            <h3 className='text-sm font-semibold'>
              RE: {String(regulacionEntradaEmpresa).padStart(6, '0')}
              {isOCNoPermitido && (
...
export default function SearchREMovimientoAlmacen(props) {
    ...
    const autoComplete = useMemo(() =>   createAutocomplete({
        placeholder: "Ingresa busqueda de Regulacion Entrada",
        onStateChange: ({ state }) =>  {
          setCriterio(state)
            ...
            getItems: async({ query }) => {
              if (!!query && query.length >= minLetras) {
                return toast.promise(
                  ...
                  fetch(`/api/inventario/movimientoAlmacen/buscar/reEmpresa?q=${query}&idempresa=${props.empresaSeleccionada.empresaRef}`)
                    .then((res) => res.json()),
                  {
