...
const AutocompleteItem = ({ _id, nombreCategoria, desc, especs, imagenUrl, setAutoCompleteState, formValues, setFormValues, setOpen }) => {
  
  const handleItemClick = () => {
    setAutoCompleteState((prevState) => ({ ...prevState, isOpen: false }));

    const nuevasEspecificaciones = especs.map((especCategoria) => ({
      especificacionRef: especCategoria.especRef,
      nombre: especCategoria.nombreEspec,
      valor: '' // Inicializamos el valor como una cadena vacía
    }));
    ...
    <li onClick={handleItemClick} >
      <div className='hover:bg-gray-200 flex gap-4 p-4'>
        <div className='w-4/5 flex justify-between ' >
          <div>
            <h3 className='text-sm font-semibold'>{nombreCategoria}</h3>
...
export default function SearchCategoriaProducto(props) {
