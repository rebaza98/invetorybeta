...
const AutocompleteItem = ({ _id, razonSocial, numeroDoc, docId, setAutoCompleteState, formValues, setFormValues, setOpen }) => {
  ...
  return (
    <li onClick={isPersonaUsed ? undefined : handleItemClick} className={isPersonaUsed ? 'cursor-not-allowed' : ''}>
      <div className='hover:bg-gray-200 flex gap-4 p-4'>
        <div>
          <h3 className='text-sm font-semibold'>{razonSocial} {isPersonaUsed && <span className="inline-flex flex-shrink-0 items-center rounded-full bg-gray-50 px-1.5 py-0.5 text-xs font-medium text-gray-700 ring-1 ring-inset ring-gray-600/20">
                  Usado
                </span>}</h3>
...
export default function SearchPersonaProducto(props) {
