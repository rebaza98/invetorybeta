...
const AutocompleteItem = ({ _id, esProducto, nombreProducto, sku, marca, modelo, esCombo, imagenUrl, setAutoCompleteState, formValues, setFormValues, setOpen }) => {
  console.log(`ES ${nombreProducto} P?= ${esProducto} C?= ${esCombo} `)
  const isProductoNoPermitido = formValues.productosCombo.some((elegida) => elegida.productoRef === _id) || (formValues?._id === _id) || !esProducto || esCombo;
...
export default function SearchProductoAdicionalProducto(props) {
            ...
            getItems: async({ query }) => {
              if (!!query && query.length >= minLetras) {
                return toast.promise(
                  fetch(`/api/inventario/producto/buscar?q=${query}`)
                    .then((res) => res.json()),
                  {
