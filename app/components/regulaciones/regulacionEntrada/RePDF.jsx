...
//     flexGrow: 1,
//   },
// });    {String(re.ocEmpresa).padStart(5, '0')}
...
const RePDF = ({re}) => {
  ...
  const retornaEstado = () => {
    ...
    if (re.estado === "PENDIENTE"){
      return "text-yellow-500"
    }
    if (re.estado === "ANULADA"){
      return "text-red-500"
    }
    if (re.estado === "INGRESADA"){
      ...
      <PDFViewer style={{ width: "100%", height: "90vh" }} >
        ...
        >
          <View style={tw("flex w-full justify-between flex-row gap-2")} >
            <View style={tw("w-3/6 p-2 text-center")} >
              ...
              <Text style={{ marginTop: 0, marginBottom: 0 }} >
                {String(re.regulacionEntradaEmpresa).padStart(5, '0')}
              </Text>
            </View>
          ...
          <View style={tw("flex w-full justify-between flex-row gap-2")} >
            <View style={tw("w-3/6 p-2")} >
              ...
              <Text style={tw("text-xs font-bold")} >
                Razon Social: {re.proveedor?.razonSocial}
              </Text>
              <Text style={tw("text-xs font-bold")} >
                {re.proveedor?.docId.nombre}: {re.proveedor?.numeroDoc}
              </Text> */}
              <Text style={tw("text-xs font-bold")} >
              ...
              {re.ingresadoAlmacen && (
                <>
                  <View style={tw("flex flex-row justify-between")} >
                    ...
                    <Text style={tw("text-xs m-0")} >
                      {String(re.movAlmacen.movimientoRef.movimientoEmpresa).padStart(5, '0')}
                    </Text>
                  </View>
                  <View style={tw("flex flex-row justify-between")} >
                    <Text style={tw("text-xs m-0")} >
                    ...
                    <Text style={tw("text-xs m-0")} >
                      {formatearFecha(re.movAlmacen.movimientoRef.fechaMovimiento)}
                    </Text>
                  </View>
                </>
              )}
          ...
          <View style={tw("border rounded-md mt-4")}>
            ...
            {/* BODY */}
            {/* LINEAS */}
            {re.productos?.map((producto, index) => (
          ...
          <View style={tw("flex mt-4 flex-row text-center justify-end")} >
            <View style={tw("w-2/5 border rounded-md mt-2 p-2")} >
              <View style={tw("text-sm flex flex-row text-left justify-between")} >
                ...
                <Text>
                  {parseFloat(re.subTotal).toFixed(2)}
                </Text>
              </View>
              <View style={tw("text-sm flex flex-row text-left justify-between")} >
                <Text>
                ...
                <Text>
                  {parseFloat(re.impuesto).toFixed(2)}
                </Text>
              </View>
              <View style={tw("text-base mt-2 flex flex-row text-left justify-between")} >
                ...
                <Text>
                  S/.{parseFloat(re.total).toFixed(2)}
                </Text>
              </View>
            </View>
